...
def start_command_handler(message: object) -> None:
    ...
    if (ufile := f"{message.from_user.id}") in os.listdir("Accounts/"):
        ...
        TempMailBot.reply_to(
            message=message,
            text=f"Welcome Back {message.from_user.first_name}.\nUse /mail to access the menu!\nBot developed by @Ashlynn_Repository",
        )
        ...
        try:
            ...
            if len(keyboard) == 1:
                TempMailBot.send_message(
                    chat_id=cid,
                    text="You currently have no emails. Create a new one using the /mail menu\n\nYou can create Email up to 99.\n(Note: Due to Telegram's limitations, a message cannot contain more than 100 buttons.)",
                    reply_markup=Markups,
                )
        ...
        try:
            ...
            with open(f"Accounts/{uid}/mails/{mail}", "r") as data:
                ...
                # Otherwise if condition is true, get result
                    ...
                    TempMailBot.send_message(
                        chat_id=cid,
                        text="Note that attachments are not supported. You can use this mail for simple OTP verification, or link verification",
